    ...
    actions: {
        ...
        isValidPassword(e) {
            ...
            if (!check_length.test(e.target.value)) {
                ...
                this.displayAlert(e.target, ' must contain at least one Uppercase letter*')
            } else if (!check_lowerCase.test(e.target.value)) {
                this.displayAlert(e.target, ' must contain at least one Lowercase letter*')
            } else if (!check_upperCase.test(e.target.value)) {
                this.displayAlert(e.target, ' must contain at least one Uppercase letter*')
            } else if (!check_special_char.test(e.target.value)) {
                this.displayAlert(e.target, ' must contain at least one special character*')
            } else if (!check_numeric.test(e.target.value)) {
                this.displayAlert(e.target, ' must contain at least one numeric digit*')
            } else this.removeAlert(e.target)
        },
        registerUser(user) {
            axios.post("https://testapi.io/api/dartya/resource/users", user)
                .then((res) => {
                    if (res.status === 201) {
