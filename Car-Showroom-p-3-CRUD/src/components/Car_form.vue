  ...
  methods: {
      ...
      check_desc_length(e) {
          if (e.target.value.length < 30) {
              this.display_alert(e.target, ' must be at lest 30 characters you are only using ' + this.$refs.car_desc.value.length + ' characters*')
          } else if (e.target.value.length > 120) {
              this.display_alert(e.target, ' must not exceed 120 characters you are using ' + this.$refs.car_desc.value.length + ' characters*')
          } else {
              this.remove_alert(e.target)
          }
      },
      is_img_url(e) {
          ...
          // })
          if (!(/^https?:\/\/.+\.(jpg|jpeg|png|webp|avif|gif|svg)$/.test(e.target.value))) {
      ...
      set_edited_car() {
          this.car.name = this.$refs.car_name.value;
          this.car.image = this.$refs.car_image.value;
          this.car.desc = this.$refs.car_desc.value;
          this.car.price = this.$refs.car_price.value;
          return
      },
      edit_car() {
          if (!this.is_valid && this.validate_form()) {
